...
{
    public float jumpForce = 20000f;  // Adjust this value as needed
    public Rigidbody rb;
    // Current level index

    public Transform[] _startPosPoint;
    int currentlevel;
    ...
    public bool _iscollideWithHalfPoint;
    ...
    private void Start()
    {
       // rb = GetComponent<Rigidbody>();
        if (GN_GameController.instance)
        currentlevel = GN_GameController.instance.currentLevel;
        if(_startPosPoint != null ) { 
        transform.position = new Vector3(_startPosPoint[currentlevel - 1].position.x, _startPosPoint[currentlevel - 1].position.y, _startPosPoint[currentlevel - 1].position.z);
        transform.rotation = _startPosPoint[currentlevel - 1].rotation;
        }
    }
    ...
    void Update()
    {
        ...
        if (_iscollideWithHalfPoint == true)
        {
            // Print out the name of the collider
            //  Debug.Log("Bike passed through collider: " + other.name);


            GN_GameController.instance._isReachedAthalflvl = true;
            _iscollideWithHalfPoint = false;
        }
        if (activeControl)
        {
        ...
        //=======================
        // Cast a ray forward from the bike's position
        /* Ray ray = new Ray(transform.position, transform.forward);
         RaycastHit hit;

         // Check if the ray hits an object with the "respawnpoints" tag
         if (Physics.Raycast(ray, out hit, Mathf.Infinity))
         {
             // Draw the raycast
             Debug.DrawRay(ray.origin, ray.direction * hit.distance, Color.green);

             if (hit.collider.CompareTag("respawnpoints"))
             {
                 // Print out the tag of the collider
                 Debug.Log("Bike passed through collider with tag: " + hit.collider.tag);
                 lastRespawnPointpos = hit.collider.transform.position;
                 lastspawnrot = hit.collider.transform.rotation;
         ...
         {
             // Draw the raycast if it doesn't hit anything
    ...
    void FixedUpdate()
    {
        ...
        if(crash && AdManagerAdmob.instance._isRewarded == true)
        {
          //  Respawn();
            Debug.Log("Respawn");
        }
        ...
        foreach (WheelComponent w in wheels)
        {
            ...
            if (col.GetGroundHit(out hit) && (w == wheels[1] || (w == wheels[0] && Wheelie == 0)))
            {
                ...
                if (bikeParticles.brakeParticlePrefab)
                {
                    ...
                    {
#if UNITY_EDITOR
                        // Check if the tag is defined (Editor only)
                        if (UnityEditorInternal.InternalEditorUtility.tags.Contains(bikeSetting.hitGround[i].tag))
#endif
                        {
    ...
    /* void OnTriggerEnter(Collider other)
     {
         if (other.CompareTag("respawnpoints"))
         {
    ...
    {
        //temp
       // BikeAnimation.instance.BikeRespawn();
        transform.position = WinAnimTrigger.instance.lastRespawnPointpos;
        transform.rotation = WinAnimTrigger.instance.lastspawnrot;
        Debug.Log("Bike is respawn");
        // You may also want to reset the bike's rotation, velocity, etc. depending on your game's requirements
        myRigidbody.constraints = RigidbodyConstraints.FreezeAll;
        myRigidbody.centerOfMass = bikeSetting.shiftCentre;

        // Also, you may want to reset the bike's velocity and angular velocity
    ...
    bool IsGrounded()
    {
        // Cast a ray downwards to check if the bike is on the ground
